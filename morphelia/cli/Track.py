...
def run(inp, out,
    ...
    # tracking
    adata = morphelia.ts.track(adata,
                               time_var=time_var,
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description=f'Clean data.')
    ...
    parser.add_argument('-g', '--group_vars', nargs='+', default=["Metadata_Well", "Metadata_Field"],
                        help='Group data by these variables to get single fields.')
    parser.add_argument('--x_loc', type=str, default="Cells_Location_Center_X",
                        help='Variable with information about x location.')
    parser.add_argument('--y_loc', type=str, default="Cells_Location_Center_Y",
                        help='Variable with information about y location.')
    parser.add_argument('--filter_tracks', type=bool, default=False,
                        help='Filter tracks by min_track_len and allowed_dummies.')
    parser.add_argument('--allowed_dummies', type=int, default=0,
                        help='Max allowed dummies in track.')
    parser.add_argument('--min_track_len', default='max',
                        help="Min track length. Takes maximum track length if 'max'.")
    parser.add_argument('--drop_untracked', type=bool, default=True,
                        help='Drop all false positive tracks from the anndata object.')
    parser.add_argument('--max_search_radius', type=int, default=100,
                        help='Local spatial search radius (pixels).')
    parser.add_argument('--field_size', nargs='+', default=[2048, 2048],
