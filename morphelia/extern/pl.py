"""
Implementation of scanpy's plotting function adjusted
to morphological data.
Original source code can be found here:
https://github.com/theislab/scanpy/blob/master/scanpy/plotting/_preprocessing.py
...
def highly_variable_genes(
    ...
    for idx, d in enumerate([var_or_disp_norm, var_or_disp]):
        ...
        if idx == 0:
            pl.legend()
        pl.xlabel(('$log_{10}$ ' if False else '') + 'mean value of features')
    ...
    sc.plotting._utils.savefig_or_show('filter_feature_dispersion', show=show, save=save)
    if show is False:
        return pl.gca()
